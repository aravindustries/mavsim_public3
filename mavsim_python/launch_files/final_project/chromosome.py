...
parent_df = pick_parents(50, 'launch_files/chap06/rmse_results7.csv')
child_df = get_children_df(parent_df, 80)
elite_df = pick_parents(20, 'launch_files/chap06/rmse_results7.csv')
population = pd.concat([elite_df, child_df], ignore_index=True)
print(population.head)
eval_df(population, 'launch_files/chap06/gen0.csv')

for gen in range(1, 50):
    parent_df = pick_parents(10, 'launch_files/chap06/gen'+str(gen-1)+'.csv')
    child_df = get_children_df(parent_df, 40)
    elite_df = pick_parents(10, 'launch_files/chap06/gen'+str(gen-1)+'.csv')
    population = pd.concat([elite_df, child_df], ignore_index=True)
    print(population.head)
    eval_df(population, 'launch_files/chap06/gen' + str(gen) + '.csv')